...
int main() {
	...
	struct sock_filter code[] = {
        // Verificar que el paquete es IPv4 (Ethertype 0x0800)
        { 0x28, 0, 0, 0x0000000c },  // Cargar el Ethertype de la cabecera Ethernet (offset 12)
        { 0x15, 0, 8, 0x00000800 },  // Comparar con 0x0800 (IPv4), si no es, saltar al final

        // Verificar que el protocolo de la cabecera IP es UDP (protocolo 17)
        { 0x30, 0, 0, 0x00000017 },  // Cargar el protocolo de la cabecera IP (offset 23)
        { 0x15, 0, 6, IPPROTO_UDP }, // Comparar con 0x11 (UDP), si no es, saltar al final

        // Verificar que el puerto de origen es 67 (DHCP Server)
        { 0x28, 0, 0, 0x00000014 },  // Cargar el inicio de la cabecera UDP (longitud de la cabecera IP)
        { 0x45, 4, 0, 0x00001fff },  // Verificar que no esté fragmentado
        { 0xb1, 0, 0, 0x0000000e },  // Calcular la longitud de la cabecera IP y ajustar x
        { 0x48, 0, 0, 0x00000000 },  // Cargar el puerto de origen (offset de 0 desde la cabecera UDP)
        { 0x15, 0, 2, SERVER_PORT }, // Comparar con 67, si no es, saltar al final

        // Verificar que el puerto de destino es 68 (DHCP Client)
        { 0x48, 0, 0, 0x00000002 },  // Cargar el puerto de destino (offset de 2 desde la cabecera UDP)
        { 0x15, 0, 1, CLIENT_PORT }, // Comparar con 68, si no es, saltar al final

        // Aceptar el paquete si cumple con todas las condiciones
        { 0x6, 0, 0, 0x00040000 },   // Aceptar el paquete (retorno de 65535 bytes)
        
        // Rechazar cualquier otro paquete
        { 0x6, 0, 0, 0x00000000 }    // Retorno de 0 bytes (descartar el paquete)
    };
	int si = 1234; 
